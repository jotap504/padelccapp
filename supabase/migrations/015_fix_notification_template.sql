-- Fix missing template column in notification INSERT statements
-- The notifications table requires 'template' to be NOT NULL

-- First, add the template column if it doesn't exist
ALTER TABLE notifications ADD COLUMN IF NOT EXISTS template TEXT NOT NULL DEFAULT 'general';

-- Fix notify_match_created function - add template column
...
-- Fix notify_match_result_entered function - add template column
